...
- **Runner agent version accuracy:** `LoadConfig` tries four strategies (symlink,
  `deps.json`, versioned bin directories, diag log). `version="unknown"` only
  occurs on installations where all four fail (e.g. Windows runners not yet
  validated).
